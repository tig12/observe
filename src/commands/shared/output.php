...
use observe\commands\shared\output\outputPage;
use observe\commands\shared\output\outputImg;
    ...
    **/
    public static function execute(array &$studyConfig, array $params): string {
        ...
        $usage = "Usage of this command: php run-observe <study> output <action> <object>\n"
            ...
            foreach(outputPage::POSSIBLE_PAGES as $k => $v){
                $usage .= str_pad("    $k:", 17) . "$v\n";
            }
            $usage .= "If <action> = \"img\", <object> can be:\n";
            foreach(outputImg::POSSIBLE_IMG as $k => $v){
                $usage .= str_pad("    $k:", 17) . "$v\n";
            }
        if(count($params) != 2){
        ...
        //
        if($params[0] == 'page'){
            $msg = outputPage::execute($studyConfig, $params);
        }
        else{
            $msg = outputImg::execute($studyConfig, $params);
        }
        return ($msg != '' ? "$msg\n$usage" : '');
    }
