...
class groupByNumber implements Command {
    ...
    public static function execute($params=[]){
        ...
        if(!isset($params['range'])){
            throw new ObserveException("$classname needs a parameter 'range'");
        }
        $range = $params['range'];
        if(!is_int($range)){
            throw new ObserveException("$classname : Parameter 'range' must be an integer");
        }
        //
